...
reset() { k -n "$NAMESPACE" delete hpa hpa-target --ignore-not-found >/dev/null; k -n "$NAMESPACE" scale deployment/hpa-target --replicas=1 >/dev/null; k -n "$NAMESPACE" rollout status deployment/hpa-target --timeout=90s >/dev/null; control 0 0.2; }
...
arm() {
  ...
  if [ "$name" = queue ]; then wait_custom vllm_num_requests_waiting 0; fi
