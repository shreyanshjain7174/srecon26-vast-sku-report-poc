...
capture_cpu() { local target=$1 temporary="$1.tmp" end=$((SECONDS+60)); while true; do if cpu > "$temporary" 2>/dev/null && jq -e '.containers[] | select(.name == "server") | .usage.cpu' "$temporary" >/dev/null; then mv "$temporary" "$target"; return 0; fi; rm -f "$temporary"; (( SECONDS < end )) || return 1; sleep 3; done; }
